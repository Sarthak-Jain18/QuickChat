...
export default function ProfilePage() {
    ...
    async function onSubmitHandler(e) {
        ...
        reader.onload = async () => {
            ...
            await updateProfile({ profilePic: base64Image, bio, name })
            navigate('/')
        }
    }
        ...
        <div className="min-h-screen bg-cover bg-no-repeat flex items-center justify-center">
            ...
            <div className="w-5/6 max-w-2xl backdrop-blur-2xl text-gray-300 border-2 border-gray-600 flex items-center justify-between max-sm:flex-col-reverse rounded-lg">
                ...
                <form onSubmit={onSubmitHandler} className="flex flex-col gap-5 p-10 flex-1">
                    ...
                    <input onChange={(e) => setName(e.target.value)} value={name} type="text" name="" id="" placeholder="Your name"
                        className="p-2 border border-gray-500 rounded-md focus:outline-none focus:ring-2 focus:ring-violet-500" required />

                    <textarea onChange={(e) => setBio(e.target.value)} value={bio} rows={4} placeholder="Write profile bio"
                        className="p-2 border border-gray-500 rounded-md focus:outline-none focus:ring-2 focus:ring-violet-500" required></textarea>
